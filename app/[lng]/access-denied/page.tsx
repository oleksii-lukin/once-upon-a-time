...
export default async function AccessDeniedPage({ params }: { params: Promise<{ lng: string }> }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 text-center">
      ...
      <h1 className="text-4xl font-bold mb-4">{t('access_denied') || 'Access Denied'}</h1>
      <p className="text-muted-foreground text-lg mb-8 max-w-md">
        {t('access_denied_message') || 'You do not have the required permissions to access this area.'}
      </p>
      <Link
        href={`/${lng}`}
