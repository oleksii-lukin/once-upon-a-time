 ...
 * Save edited image back to local file system with enhanced security validation
 */
async function handleSaveImage(request: NextRequest, config: EnvironmentConfig) {
  ...
  try {
    ...
    if (!sanitizedParams.filePath) {
      securityLogger.logValidationError('filePath', filePath, 'No file path provided', requestId)
      return NextResponse.json(
    ...
    const pathInfo = await fileSystemHandler.validateAndResolvePath(sanitizedParams.filePath)

    if (pathInfo.exists && !sanitizedParams.overwrite) {
      securityLogger.log({
        level: 'info',
        category: 'file_access',
        event: 'overwrite_denied',
        requestId,
        details: {
          filePath: sanitizedParams.filePath,
          reason: 'File exists and overwrite not specified',
        },
      })
    ...
    // Save the image
    await fileSystemHandler.writeImageFile(sanitizedParams.filePath, imageBuffer)
    ...
    const updatedPathInfo = await fileSystemHandler.validateAndResolvePath(sanitizedParams.filePath)
    const stats = await import('fs/promises').then(fs => fs.stat(updatedPathInfo.absolute))
    ...
    // Handle network/connection errors (for graceful handling of interruptions)
...
export const POST = withEnvironmentGuard(handleSaveImage)
