 ...
 */
/**
 * Core logic for timer extension check - separated from XState for reuse
 */
export async function executeTimerExtension(input: TimerExtensionInput): Promise<TimerExtensionOutput> {
...
export const timerExtensionActor = fromPromise(async ({ input }: { input: TimerExtensionInput }): Promise<TimerExtensionOutput> => {
  return executeTimerExtension(input)
})
